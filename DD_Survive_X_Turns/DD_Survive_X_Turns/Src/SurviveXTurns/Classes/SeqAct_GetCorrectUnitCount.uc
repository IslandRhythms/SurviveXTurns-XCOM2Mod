...
class SeqAct_GetCorrectUnitCount extends SequenceAction;
...
function protected int GetFilteredUnitCount(out array<XComGameState_Unit> Units)
{
	...
	`log("###########################################################################################");
	`redscreen("HELLLO IS THIS THING ON");
	if(CharacterTemplateFilter == "" && TestVolume == none)
	{
		foreach Units(UnitState) {
		`log("IFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF");
		`log("Looking at unit " $ UnitState.GetSoldierClassTemplateName());
		}
		return Units.Length;
	}
	else
	{
		`log("EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEELSE");
		WorldData = `XWORLD;
		...
		`log("Looking at unit " $ UnitState.GetSoldierClassTemplateName());
			...
			if(TemplateName == '' || UnitState.GetMyTemplateName() == TemplateName)
			{
				...
				if(TestVolume == None || TestVolume.EncompassesPoint(WorldData.GetPositionFromTileCoordinates(UnitLocation)))
				{
					...
					FilteredCount++;
					if(UnitState.GetSoldierClassTemplateName() == 'Specialist') {
					`log("BEfore INcrement " $ UnitState.GetSoldierClassTemplateName());
						Gremlins++;
					}
				}
			}
		}
		`log("This is the value of filtered count");
		`log(FilteredCount);
		FilteredCount = FilteredCount - Gremlins;
...
defaultproperties
{
	ObjName="Get Correct Unit Count";
	ObjCategory="Unit";
	bCallHandler=false;
